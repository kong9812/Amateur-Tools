...
int						g_nStage = STAGE_TUTORIAL;	// ステージ番号
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		if (OpenSound(hWnd) == FALSE)
		{
			// エラーメッセージ
			char lpMsgBuf[ERROR_MSG_MAX] = 
				"WAVのパスが取得できません～\n"
				"嘘じゃないよ～（*´▽｀*）\n"
				"では先に失礼します～";
			MessageBox(hWnd, (LPCTSTR)lpMsgBuf, _TEXT("音データがないやん！エラー～ラ～"), MB_OK | MB_ICONINFORMATION);
	...
	case WM_KEYDOWN:
		...
		case VK_F1:
			...
			// カレントディレクトリを設定する
			SetCurrentDirectory(GetOldPath());
			...
			InitPlayerControl(RE_INIT);
			song->soundBuffer = LoadSound(SOUND_2D);
			break;
...
HRESULT Init(HWND hWnd, BOOL bWindow)
{
	...
	// Init処理
	{
		...
		// プレイヤーのマネージャー処理
		InitPlayerManager(FIRST_INIT);

		// 波のマネージャー処理
		InitWaveManager(FIRST_INIT);

		//3Dのマネージャー処理
		Init3dManager(FIRST_INIT);

		// チュートリアル処理
		InitTutorial(FIRST_INIT);
...
void Update(HWND hWnd)
{
	...
	case STAGE_TUTORIAL:
		...
		break;
	case STAGE_PLAYER:				// プレイヤー
		// プレイヤーのマネージャー処理
		UpdatePlayerManager(hWnd);
		break;
	case STAGE_WAVE:				// 波
		// 波のマネージャー処理
		UpdateWaveManager(hWnd);
		break;
	case STAGE_3D:					// 3Dモード(モノ限定)
		// 3Dのマネージャー処理
		Update3dManager(hWnd);
		break;
	case STAGE_REVERB:
		// 環境リバーブ
		UpdateReverb(hWnd);
